...
server <- function(input, output, session) {
  ...
  main_ui <- function() {
    fluidPage(
      ...
      wellPanel(
        h4("COP 23 Export Figures"),
        DT::dataTableOutput("stats_COP"),
        strong("Download to paste into AGYW tab for COP"),
        br(),
        downloadButton("blankTemplateDownloadCOP23Export",
                       "Download COP 23 Export",
                       style="color: #fff; background-color: #20A39E; border-color: #1C110A")),
      wellPanel(
        h4("Numerator"),
  ...
  output$blankTemplateDownloadCOP23Export <- downloadHandler(
    filename = function() {
      paste("COP23Export_AGYW_Tab", 
            ".xlsx",
            sep = "")
    },
